...
class CircularQueue
  ...
  def enqueue(object)
    target = (@newest_index + 1) % @buffer_size
    @oldest_index = (@oldest_index + 1) % @buffer_size unless @queue[target] == nil
    @queue[target] = object
  ...
  def dequeue
    ...
    @oldest_index = (@oldest_index + 1) % @buffer_size unless old_value == nil
    old_value
  end
end
