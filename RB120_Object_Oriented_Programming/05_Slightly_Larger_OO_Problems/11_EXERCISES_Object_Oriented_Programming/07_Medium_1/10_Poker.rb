...
class PokerHand
  def initialize(deck)
    @hand = []
    @rank_counter = Hash.new(0)
    5.times do
      new_card = deck.draw
      @hand << new_card
      @rank_counter[new_card.rank] += 1
    end
  end
  ...
  def n_of_a_kind?(number)
    @rank_counter.one? { |_, count| count == number }
  end
  ...
  def full_house?
    n_of_a_kind?(3) && @rank_counter.one? { |_, count| count == 2 }
  end
  ...
  def straight?
    return false if @rank_counter.any? { |_, count| count > 1 }
  ...
  def two_pair?
    @rank_counter.select { |_, count| count == 2 }.size == 2
  end

  def pair?
    @rank_counter.count { |_, count| count == 2 } == 1
  end
end
